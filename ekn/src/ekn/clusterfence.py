...
async def require(api: Api, declared: str | None, *, override: bool = False) -> str | None:
    ...
    if declared is None:
        if override:
            ...
            return actual
        raise SystemExit(
            f"refusing to act: this configuration does not say which cluster it is for, "
            f"and the ambient kubeconfig points at {actual}.\n\n"
            f"{_adopt(actual)}\n"
    ...
    if declared != actual:
        raise SystemExit(
            f"refusing to act: this configuration is for cluster {declared}, "
